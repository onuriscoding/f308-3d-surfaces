...
// ------- constructor ------- 
Perlin2D::Perlin2D(float initScale , int nUnique, int theta) : Perlin(initScale, nUnique, 10) {
    initPermuation();
}

void Perlin2D::updateMesh(ofMesh &mesh, std::vector<int> values){
	int width = values.get(0), height = values.get(1);
      ...
      for (int y = 0 ; y < height; y ++){
        for (int x = 0 ; x < width; x ++){
            ...
            float nx = x * scale; 
            float ny = y * scale;
			std::vector<float> n= {nx,ny};
            
            float noiseValue = rotatePerlinNoise(n); 
...
float Perlin2D::grad(int hash , std::vector<float> coord){
    int h = hash%3;
	float x = coord.get(0), y = coord.get(1);
    if (h == 0) return x + y;
    if (h == 1) return -x + y ;
    if (h == 2) return x - y;
    return -x - y;
...
float Perlin2D::rotatePerlinNoise(std::vector<float> coord){
...
float Perlin2D::perlin(std::vector<float> coord) {
